...
3. Compile: `make build` (60 Hz plus, default), `make build APP=pattern`, `make build APP=term`, `make build APP=demos`, `make build APP=beam`, or `make build APP=hello` (bring-up).
4. Gate: `make test APP=hello` (unique `digest=` + `pong`). 60/78 Hz measure: `make test` (`crt-cross60` banner) then `make monitor` (keys `1`–`4`, `m` 80/132, `r` 60/78, `a`/`d`, `w`/`s`). Analog drawings: `make test APP=pattern` (same `m`/`r`; keys `1`–`6`). Glass TTY: `make test APP=term` (BOOTSEL cycles 78/60 × 80/132; host bytes stay the session). Phosphor reel: `make test APP=demos` (CDC `2` → `scene=radar`; same `m`/`r`; scenes `1`–`6`). Beam-line tests: `make test APP=beam` (CDC `?` → `line=` and `then=` differ; keys `1`–`5`). CDC attach for any running image: `make monitor`.
