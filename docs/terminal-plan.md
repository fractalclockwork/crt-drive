...
Sources: [`video/`](../video/) (shared 78 Hz 80-col scanout), [`term/`](../term/) (`crt_term` UF2). Do not mix host bytes with pattern CDC keys.
...
Two UF2s from one CMake project:

| Target | Role | CDC |
| --- | --- | --- |
| `crt_drive` | Test patterns | `1`–`5` / BOOTSEL; `pattern=` banner |
| `crt_term` | Glass TTY | host bytes; `crt-term digest=` banner |
...
**Decided for this slice:** [`term/font_7x10.c`](../term/font_7x10.c), U+0020–U+007E, 10 bytes/glyph, bit 6 = left column (same packing as the pattern `H`). `.notdef` for everything else.
